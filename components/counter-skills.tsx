        ...
        <div className="grid justify-between max-w-3xl grid-cols-2 gap-3 mx-auto my-8 md:grid-cols-4 md:gap-6">
            ...
            {dataCounter.map(({ id, endCounter, text, lineRight, lineRightMobile }) => (
                ...
                <div key={id} className={`${lineRight && 'ltr'}`}>
                    ...
                    `}>
                        <p className="flex mb-2 text-3xl font-extrabold md:text-4xl text-secondary">+ <CountUp end={endCounter} start={0} duration={5} /></p>

                        <p className="text-xs uppercase max-m-[100px]">{text}</p>
                    </div>
                </div>
            ))}
